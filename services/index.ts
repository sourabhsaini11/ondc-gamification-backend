      ...
      .on("end", async () => {
        try {
          ...
          if (newOrders.length > 0) {
            ...
            console.log("processedNewOrders", JSON.stringify(processedNewOrders))
            await bulkInsertDataIntoDb(processedNewOrders)
          }

          if (cancellations.length > 0) {
            console.log("cancellations", JSON.stringify(cancellations))
            const processedCancelOrders = await processCancellations(cancellations)
            console.log("processedCancelOrders", JSON.stringify(processedCancelOrders))
  ...
  try {
    ...
    for (const row of orders) {
      try {
        ...
        console.log("timestampCreated", timestampCreated, row.timestamp_created)
  ...
  try {
    for (const row of cancellations) {
      try {
        ...
        console.log("row.last_streak_date", row.last_streak_date, row, originalOrder, last_streak_date)
  ...
  try {
    ...
    if (lastStreakDate) {
      ...
      console.log(
        "dayDifference",
        dayDifference,
        lastStreakDay.format(),
        currentDay.format(),
        currentTimestamp.format(),
      )
