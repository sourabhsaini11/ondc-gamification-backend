    ...
    stream
      .on("data", (row) => {
        try {
          ...
          //   const existingStatuses = existingOrdersMap.get(orderId)
          //   if (existingStatuses?.has(orderStatus)) {
          //     const existingBuyerAppId = recordMap.get(orderId as string)?.buyerAppId

          //     if (existingBuyerAppId && existingBuyerAppId === String(userId)) {
          //       return reject({
          //         success: false,
          //         message: `Duplicate order with similar status: ${orderStatus} already present for the same buyer_app_id at index:${rowCount}`,
          //       })
          //     }
          //   }
          // }
    ...
    for (const row of cancellations) {
      try {
        ...
        if (orderStatus === "cancelled") {
          ...
          console.log("gmvAdjustment", gmvAdjustment)
          // rewardledgerUpdate(
          //   gameId,
          //   orderId,
          //   gmvAdjustment,
...
const deductStreakPointsForFutureOrders = async (
  ...
  try {
    console.log(uid, orderId, timestamp_created, game_id, streak_count)
...
const deductPointsForHigherSameDayOrders = async (
  ...
  try {
    ...
    if (updatedOrders.count > 0) {
      await Promise.all(
        allOrders.map(
          (order: any) => console.log("orders", order),
